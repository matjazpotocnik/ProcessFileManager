...
/* PROTECTED METHODS *************************************************************************** */
	...
	protected function _viewContent($Entry) {
		list($language, $content, $encoding) = $this->_getContent($Entry);
		$content = preg_replace("/\r?\n|\r/", '\n', addcslashes($content, "'\\"));
		$encoding = addslashes($encoding);
		print "text:{lang:'$language',content:'$content',encoding:'$encoding'}";
	}
	 ...
	 *
	 * @param FM_Entry $Entry		file entry object
	 * @return array				script language, file content, file encoding
	 */
	protected function _getContent($Entry) {
		$file = $Entry->getFile();
		$content = FM_Tools::readLocalFile($file);
		$encoding = FM_Tools::getEncoding($content);
		if(!$encoding) $encoding = $this->_FileManager->encoding;
		$content = htmlspecialchars($content, ENT_COMPAT, $encoding);

		if(!FM_Tools::isUtf8($content)) {
			$content = FM_Tools::utf8Encode($content, $encoding);
		}
		...
		return array($language, $content, $encoding);
	}
}
