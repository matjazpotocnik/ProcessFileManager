...
class JUpload {
	...
	public function JUpload($appletparams = array(), $classparams = array()) {
		...
		if (!isset($appletparams['sendMD5Sum'])) {
			$appletparams['sendMD5Sum'] = false;
		}
		...
		if (!isset($classparams['spaces_in_filenames']))         // set to true to convert spaces in filenames to _
			$classparams['spaces_in_filenames'] = false;
	...
	private function dstfinal(&$name, &$subdir) {
		...
		if ($this->classparams['spaces_in_filenames']) {
   ...
   */
	function receive_uploaded_files() {
			...
			if ($this->appletparams['sendMD5Sum']  &&  !isset($_POST['md5sum'])) {
			...
			if ($this->appletparams['sendMD5Sum']  && !is_array($md5sums)) {
