...
var fmTools = {
	...
	setUnselectable: function(node, cursorType) {
		node.unselectable = true;
		node.style.MozUserSelect = 'none';
		node.style.webkitUserSelect = 'none';
		node.onmousedown = function() { return false; }
		node.style.cursor = cursorType ? cursorType : 'default';
	},
	...
	toBytes: function(val) {
		...
		switch(RegExp.$3) {
			...
			case 'T': return parseFloat(RegExp.$1) * 1024 * 1024 * 1024 * 1024;
		}
		return val;
	},

	bytes2string: function(bytes, dec) {
		...
		for(var key in sizes) {
			if(bytes >= sizes[key]) {
				return this.numberFormat(bytes / sizes[key], dec) + ' ' + key;
			}
		}
		return bytes + ' B';
	},
